...
def statusDownload(mt):
    ...
    syslog.syslog(syslog.LOG_NOTICE, "CNL serial: {0}\n".format(mt.deviceSerial))
    syslog.syslog(syslog.LOG_NOTICE, "### Pump status ###")
    syslog.syslog(syslog.LOG_NOTICE, "time:                 {0}".format(mt.pumpTime))
    syslog.syslog(syslog.LOG_NOTICE, "drift:                {0}".format(mt.pumpTimeDrift))
    syslog.syslog(syslog.LOG_NOTICE, "suspended:            {0}".format(status.isPumpStatusSuspended))
    syslog.syslog(syslog.LOG_NOTICE, "bolusingNormal:       {0}".format(status.isPumpStatusBolusingNormal))
    syslog.syslog(syslog.LOG_NOTICE, "bolusingSquare:       {0}".format(status.isPumpStatusBolusingSquare))
    syslog.syslog(syslog.LOG_NOTICE, "bolusingDual:         {0}".format(status.isPumpStatusBolusingDual))
    syslog.syslog(syslog.LOG_NOTICE, "deliveringInsulin:    {0}".format(status.isPumpStatusDeliveringInsulin))
    syslog.syslog(syslog.LOG_NOTICE, "tempBasalActive:      {0}".format(status.isPumpStatusTempBasalActive))
    syslog.syslog(syslog.LOG_NOTICE, "cgmActive:            {0}\n".format(status.isPumpStatusCgmActive))
    syslog.syslog(syslog.LOG_NOTICE, "### Pump alert ###")
    syslog.syslog(syslog.LOG_NOTICE, "alertOnHigh:          {0}".format(status.isPlgmAlertOnHigh))
    syslog.syslog(syslog.LOG_NOTICE, "alertOnLow:           {0}".format(status.isPlgmAlertOnLow))
    syslog.syslog(syslog.LOG_NOTICE, "alertBeforeHigh:      {0}".format(status.isPlgmAlertBeforeHigh))
    syslog.syslog(syslog.LOG_NOTICE, "alertBeforeLow:       {0}".format(status.isPlgmAlertBeforeLow))
    syslog.syslog(syslog.LOG_NOTICE, "alertSuspend:         {0}".format(status.isPlgmAlertSuspend))
    syslog.syslog(syslog.LOG_NOTICE, "alertSuspendLow:      {0}".format(status.islgmAlertSuspendLow))
    syslog.syslog(syslog.LOG_NOTICE, "alert:                {0}".format(status.alert))
    syslog.syslog(syslog.LOG_NOTICE, "alertDate:            {0}".format(status.alertDate.strftime( "%c" )))
    syslog.syslog(syslog.LOG_NOTICE, "isAlertSilenceHigh:   {0}".format(status.isAlertSilenceHigh))
    syslog.syslog(syslog.LOG_NOTICE, "isAlertSilenceHighLow:{0}".format(status.isAlertSilenceHighLow))
    syslog.syslog(syslog.LOG_NOTICE, "isAlertSilenceAll:    {0}".format(status.isAlertSilenceAll))
    syslog.syslog(syslog.LOG_NOTICE, "alertSilenceMinutesRemaining:{0}\n".format(status.alertSilenceMinutesRemaining))
    syslog.syslog(syslog.LOG_NOTICE, "### Sensor status ###")
    syslog.syslog(syslog.LOG_NOTICE, "calibrating:          {0}".format(status.isSensorStatusCalibrating))
    syslog.syslog(syslog.LOG_NOTICE, "calibrationComplete:  {0}".format(status.isSensorStatusCalibrationComplete))
    syslog.syslog(syslog.LOG_NOTICE, "exception:            {0}".format(status.isSensorStatusException))
    syslog.syslog(syslog.LOG_NOTICE, "sensorCalMinutesRemaining:   {0}".format(status.sensorCalMinutesRemaining))
    syslog.syslog(syslog.LOG_NOTICE, "sensorBatteryLevelPercentage:{0}".format(status.sensorBatteryLevelPercentage))
    syslog.syslog(syslog.LOG_NOTICE, "sensorRateOfChange:          {0}\n".format(status.sensorRateOfChange))
    syslog.syslog(syslog.LOG_NOTICE, "### Bolus ###")
    syslog.syslog(syslog.LOG_NOTICE, "bolusingDelivered:        {0}".format(status.bolusingDelivered))
    syslog.syslog(syslog.LOG_NOTICE, "bolusingMinutesRemaining: {0}".format(status.bolusingMinutesRemaining))
    syslog.syslog(syslog.LOG_NOTICE, "bolusingReference:        {0}".format(status.bolusingReference))
    syslog.syslog(syslog.LOG_NOTICE, "lastBolusAmount:          {0}".format(status.lastBolusAmount))
    syslog.syslog(syslog.LOG_NOTICE, "lastBolusTime:            {0}".format(status.lastBolusTime.strftime( "%c" )))
    syslog.syslog(syslog.LOG_NOTICE, "lastBolusReference:       {0}".format(status.lastBolusReference))
    syslog.syslog(syslog.LOG_NOTICE, "recentBolusWizard:        {0}".format(status.recentBolusWizard))
    syslog.syslog(syslog.LOG_NOTICE, "recentBGL:                {0}\n".format(status.recentBGL))
    syslog.syslog(syslog.LOG_NOTICE, "### Basal ###")
    syslog.syslog(syslog.LOG_NOTICE, "activeBasalPattern:       {0}".format(status.activeBasalPattern))
    syslog.syslog(syslog.LOG_NOTICE, "activeTempBasalPattern:   {0}".format(status.activeTempBasalPattern))
    syslog.syslog(syslog.LOG_NOTICE, "currentBasalRate:         {0}".format(status.currentBasalRate))
    syslog.syslog(syslog.LOG_NOTICE, "tempBasalRate:            {0}".format(status.tempBasalRate))
    syslog.syslog(syslog.LOG_NOTICE, "tempBasalPercentage:      {0}".format(status.tempBasalPercentage))
    syslog.syslog(syslog.LOG_NOTICE, "tempBasalMinutesRemaining:{0}".format(status.tempBasalMinutesRemaining))
    syslog.syslog(syslog.LOG_NOTICE, "basalUnitsDeliveredToday: {0}\n".format(status.basalUnitsDeliveredToday))
    syslog.syslog(syslog.LOG_NOTICE, "### Battery ###")
    syslog.syslog(syslog.LOG_NOTICE, "batteryLevelPercentage:   {0}\n".format(status.batteryLevelPercentage))
    syslog.syslog(syslog.LOG_NOTICE, "### Insulin ###")
    syslog.syslog(syslog.LOG_NOTICE, "insulinUnitsRemaining:    {0}".format(status.insulinUnitsRemaining))
    syslog.syslog(syslog.LOG_NOTICE, "minutesOfInsulinRemaining:{0}".format(status.minutesOfInsulinRemaining))
    syslog.syslog(syslog.LOG_NOTICE, "activeInsulin:            {0}\n".format(status.activeInsulin))
    syslog.syslog(syslog.LOG_NOTICE, "### BGL ###")
    syslog.syslog(syslog.LOG_NOTICE, "sensorBGL:                {0}".format(status.sensorBGL))
    syslog.syslog(syslog.LOG_NOTICE, "sensorBGLTimestamp:       {0}".format(status.sensorBGLTimestamp))
    syslog.syslog(syslog.LOG_NOTICE, "trendArrow:               {0}\n".format(status.trendArrow))
...
def historyDownload(mt):
    ...
    syslog.syslog(syslog.LOG_NOTICE, "pumpStart:                {0}".format(historyInfo.datetimeStart))
    syslog.syslog(syslog.LOG_NOTICE, "pumpEnd:                  {0}".format(historyInfo.datetimeEnd))
    syslog.syslog(syslog.LOG_NOTICE, "pumpSize:                 {0}\n".format(historyInfo.historySize))
